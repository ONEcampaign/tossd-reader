...
2. **Check sub-pillar coverage before you compare years.** Sub-pillar tagging rolled out gradually starting in 2023. `tossd_subpillar` is `NA` on any Pillar II row the reporting provider left untagged, so `.notna()` gives the tagged share directly.
...
5. **Retrieve every Pillar II row with `pillars=2`.** This retrieves all Pillar II activities across all available years, including the 2022 baseline records and untagged 2023 rows, without year narrowing or warnings.
   ...
   print(
       p2.groupby("tossd_subpillar", observed=True, dropna=False)["usd_disbursement"]
       .sum()
       .round(1)
   )
   ```
...
```python
tagged = p2["tossd_subpillar"].notna().sum()
untagged = p2["tossd_subpillar"].isna().sum()
